...
export default {
  async fetch(request, env) {
    ...
    if (action === "notes-get") {
      ...
      return json({ ok: true, notes: await readNotes(readyDb, email) });
    }
    ...
    if (action === "notes-clear") {
      if (!readyDb) return json({ error: "D1 binding missing" }, 500);
      if (!email) return json({ error: "email required" }, 400);
      await readyDb.prepare("DELETE FROM shared_notes WHERE email = ?").bind(email).run();
    ...
    if (action === "session-start") {
      ...
      const payload = {
        ...
        voice_agent: {
          id: who.voice_agent_id,
          dynamic_variables: notes ? { shared_notes: notes } : {},
        },
        dynamic_variables: notes ? { shared_notes: notes } : {},
        memory: null,
      };
